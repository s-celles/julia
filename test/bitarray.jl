...
@testset "utility functions" begin
    ...
    for (sz,T) in allsizes
        ...
        b2 = similar(b1)
        u1 = Array(b1)
        @check_bit_operation memcopy!(b2, b1) T
        @check_bit_operation memcopy!(b2, u1) T
    end
                    ...
                    for i1 in [1; 10; 53:65; 1013:1015; 1020:1025], i2 in [1; 3; 10; 511:513], l in [1; 5; 10; 511:513; 1023:1025]
                        for fb! in [b->fill!(b, false), b->rand!(b)]
                            ...
                            if i1 < 1 || i1 > n || (i2 + l - 1 > m) || (i1 + l - 1 > n)
                                @test_throws BoundsError memcopy!(b1, i1, u1, i2, l)
                            else
                                @check_bit_operation memcopy!(b1, i1, u1, i2, l) BitArray
                                @check_bit_operation memcopy!(b1, i1, c1, i2, l) BitArray
                            end
                        end
                    end
                end
...
@testset "Indexing" begin
    ...
    @testset "multidimensional" begin
        ...
        m1, m2 = rand_m1m2()
        b2 = bitrand(m1, m2)
        @check_bit_operation memcopy!(b1, b2) BitMatrix
        ...
        for p1 = [rand(1:v1) 1 63 64 65 191 192 193]
            for p2 = [rand(1:v1) 1 63 64 65 191 192 193]
                for n = 0 : min(v1 - p1 + 1, v1 - p2 + 1)
                    b1 = bitrand(v1)
                    b2 = bitrand(v1)
                    @check_bit_operation memcopy!(b1, p1, b2, p2, n) BitVector
                end
            end
        end
    end
