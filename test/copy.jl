...
@testset "memcopy!" begin
    for (dest, src, bigsrc, emptysrc, res) in [
        ...
        @test memcopy!(copy(dest), src()) == res[1]
        @test memcopy!(copy(dest), 1, src()) == res[1]
        @test memcopy!(copy(dest), 2, src(), 2) == res[2]
        @test memcopy!(copy(dest), 2, src(), 2, 1) == res[2]

        @test memcopy!(copy(dest), 99, src(), 99, 0) == dest

        @test memcopy!(copy(dest), 1, emptysrc()) == dest
        x = emptysrc()
        exc = isa(x, AbstractArray) ? BoundsError : ArgumentError
        @test_throws exc memcopy!(dest, 1, emptysrc(), 1)

        for idx in (0, 4)
            @test_throws BoundsError memcopy!(dest, idx, src())
            @test_throws BoundsError memcopy!(dest, idx, src(), 1)
            @test_throws BoundsError memcopy!(dest, idx, src(), 1, 1)
            x = src()
            exc = isa(x, AbstractArray) ? BoundsError : ArgumentError
            @test_throws exc memcopy!(dest, 1, x, idx)
            x = src()
            exc = isa(x, AbstractArray) ? BoundsError : ArgumentError
            @test_throws exc memcopy!(dest, 1, x, idx, 1)
        end

        @test_throws ArgumentError memcopy!(dest, 1, src(), 1, -1)

        @test_throws BoundsError memcopy!(dest, bigsrc())

        @test_throws BoundsError memcopy!(dest, 3, src())
        @test_throws BoundsError memcopy!(dest, 3, src(), 1)
        @test_throws BoundsError memcopy!(dest, 3, src(), 1, 2)

        @test_throws BoundsError memcopy!(dest, 1, src(), 2, 2)
    end
end
...
@testset "with CartesianRange" begin
    ...
    let A = reshape(1:6, 3, 2), B = zeros(8,8)
        RA = CartesianRange(indices(A))
        memcopy!(B, CartesianRange((5:7,2:3)), A, RA)
        @test B[5:7,2:3] == A
        B[5:7,2:3] = 0
        @test all(x->x==0, B)
    end
