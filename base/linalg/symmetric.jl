...
function memcopy!(dest::Symmetric, src::Symmetric)
    if src.uplo == dest.uplo
        memcopy!(dest.data, src.data)
    else
...
function memcopy!(dest::Hermitian, src::Hermitian)
    if src.uplo == dest.uplo
        memcopy!(dest.data, src.data)
    else
...
*(A::HermOrSym, B::HermOrSym) = A * memcopy!(similar(parent(B)), B)
