...
function authenticate_ssh(libgit2credptr::Ptr{Ptr{Void}}, p::CredentialPayload, username_ptr)
    cred = p.credential::SSHCredentials
    revised = false
...
function authenticate_userpass(libgit2credptr::Ptr{Ptr{Void}}, p::CredentialPayload)
    cred = p.credential::UserPasswordCredentials
    revised = false
...
function credentials_callback(libgit2credptr::Ptr{Ptr{Void}}, url_ptr::Cstring,
    ...
    if isset(allowed_types, Cuint(Consts.CREDTYPE_SSH_KEY))
        if p.credential === nothing || !isa(p.credential, SSHCredentials)
            p.credential = SSHCredentials(p.username)
        end
    ...
    if isset(allowed_types, Cuint(Consts.CREDTYPE_USERPASS_PLAINTEXT))
        if p.credential === nothing || !isa(p.credential, UserPasswordCredentials)
            p.credential = UserPasswordCredentials(p.username)
        end
